...
public class UserUpdateRolesService(IServiceProvider serviceProvider) : IUserUpdateRolesService
{
    ...
    {
        if (newRoles.IsNullOrEmpty()) return BadRequestResult(Resources.EmptyRolesMessage);
    ...
    private async Task AddUserRoles(List<string> newRoles, User user)
    {
        ...
        var validRoles = newRoles.Select(r => allRoles.SingleOrDefault(role => role.Name == r)).ToList();
